...
class CycleGANModel(BaseModel):
    ...
    def backward_G(self):
        ...
        if self.opt.gan_mode != 'relhinge':
            ...
            self.loss_G_A = self.criterionGAN(self.netD_A(self.fake_B), None, other_pred=self.netD_A(self.real_B).detach())

        # GAN loss D_B(G_B(B))
        if self.opt.gan_mode != 'relhinge':
            self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True)
        else:
            self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), None, other_pred=self.netD_B(self.real_A).detach())
